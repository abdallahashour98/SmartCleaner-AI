...
class InpainterConfig:
    ...
    def export_to_conf(
        ...
        # Automatically adjust the inpainting settings based on the image content.
        # When enabled, the minimum standard deviation threshold is calculated dynamically.
        inpainting_auto = {self.inpainting_auto}

        # The minimum standard deviation of colors around the edge of a given mask
        # to perform inpainting on the region around the mask.
        # [GUI: <br>]If this value matches the maximum deviation for masks, only failed bubbles will be inpainted,
    ...
    def import_from_conf(self, config_updater: cu.ConfigUpdater) -> None:
        ...
        try_to_load(self, config_updater, section, bool, "inpainting_auto")
...
class Config:
    ...
    def save(self, config_path: Path | None = None) -> bool:
        ...
        # Gemini API Key for AI automated translation matching
        gemini_api_key = {none_to_empty(self.gemini_api_key)}
    ...
    def from_config_updater(cls, conf_updater: cu.ConfigUpdater) -> "Config":
        ...
        try_to_load(config, conf_updater, section, str | None, "gemini_api_key")
