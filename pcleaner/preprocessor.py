...
def prep_json_file(
    ...
    if ocr_engine_factory is not None:
        page_data, analytic = ocr_check(
            page_data,
            ocr_engine_factory,
            preprocessor_conf.ocr_max_size if not record_all else 999_999_999,
...
def ocr_check(
    ...
    box_sizes = []
    discarded_box_sizes = []
    discarded_box_texts: list[tuple[str, st.Box, str]] = []

    def process_box(i: int) -> tuple[int, bool, str, tuple]:
        box = boxes[i]
        lang = langs[i]
        ...
        scaled_box_data = (text, box.scale(1 / scale), "")
        return i, remove, text, scaled_box_data

    from concurrent.futures import ThreadPoolExecutor
    
    # Force PIL to load the image into memory. Calling crop() concurrently on a lazy-loaded
    # image can cause OSErrors due to thread safety issues with the underlying file descriptor.
    base_image.load()
    
    with ThreadPoolExecutor() as executor:
        results = executor.map(process_box, candidate_small_bubble_indices)

    for i, remove, text, scaled_box_data in results:
        box = boxes[i]
        box_sizes.append(box.area)
        
        if record_all:
            discarded_box_texts.append(scaled_box_data)
        
        if remove and not record_all:
    ...
    return (
        ...
        st.OCRAnalytic(
            ...
            discarded_box_sizes,
            discarded_box_texts,
            discarded_box_texts if record_all else [],
        ),
    )
