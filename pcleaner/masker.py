...
def mask_page(m_data: st.MaskerData) -> Sequence[st.MaskFittingAnalytic]:
    ...
    if len(page_data.merged_extended_boxes) != len(page_data.reference_boxes):
        logger.warning(
            f"Box count mismatch: {len(page_data.merged_extended_boxes)} merged vs "
            f"{len(page_data.reference_boxes)} reference boxes for {original_path}"
        )
