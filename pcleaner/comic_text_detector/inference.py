...
from .basemodel import TextDetBase, TextDetBaseDNN, TextDetBaseORT
...
class TextDetector:
    ...
    def __init__(
        ...
        if Path(model_path).suffix == ".onnx":
            ...
            try:
                ...
                avail_provs = ort.get_available_providers()
                if "DmlExecutionProvider" in avail_provs or "CUDAExecutionProvider" in avail_provs:
                    self.net = TextDetBaseORT(input_size, model_path)
                    self.backend = "ort"
                    use_ort = True
            except Exception:
                use_ort = False
    ...
    def __call__(self, img, refine_mode=REFINEMASK_INPAINT, keep_undetected_mask=False):
        ...
        if mask.shape[1] == 2:  # some backend/OpenCV versions spit out reversed result
            tmp = mask
            mask = lines_map
            lines_map = tmp
        ...
        # 0.5 is safe. Lower values cause CPU segmentation explosion. Smart chunking already catches small bubbles.
        box_thresh = 0.5
