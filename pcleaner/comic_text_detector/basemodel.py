...
class TextDetBase(nn.Module):
    def __init__(self, model_path, device="cpu", half=False, fuse=True, act="leaky"):
...
class TextDetBaseORT:
    """
    High-performance ONNX Runtime inference runner for ComicTextDetector.
    Leverages GPU acceleration (NVIDIA CUDA or Windows DirectML) for 4-5x faster inference.
    """
    def __init__(self, input_size, model_path, providers=None):
        ...
        self.active_provider = self.sess.get_providers()[0] if self.sess.get_providers() else "unknown"
