...
class ProfileOptionWidget(Qw.QHBoxLayout):
    ...
    def set_data_enabled(self, enabled: bool) -> None:
        self._data_widget.setEnabled(enabled)
...
class ProfileToolBox(Qw.QToolBox):
    ...
    def load_profile_structure(self, structure: list[ProfileSection]) -> None:
        ...
        # Wire up dependencies between settings.
        if "inpainter" in self._widgets:
            inpainter = self._widgets["inpainter"]
            if "inpainting_auto" in inpainter:
                auto_widget = inpainter["inpainting_auto"]
                manual_settings = [
                ...
                def update_manual_state() -> None:
                    is_auto = auto_widget.get_value()
                    for key in manual_settings:
                        if key in inpainter:
                            inpainter[key].set_data_enabled(not is_auto)

                # Connect the signal to update the enabled state.
                auto_widget.valueChanged.connect(update_manual_state)
    ...
    def set_profile_values(self, profile: cfg.Profile, no_new_defaults: bool = False) -> None:
        ...
        # Ensure the initial state of dependent widgets is correct.
        if "inpainter" in self._widgets:
            inpainter = self._widgets["inpainter"]
            if "inpainting_auto" in inpainter:
                is_auto = inpainter["inpainting_auto"].get_value()
                manual_settings = [
                    "inpainting_min_std_dev",
                    "inpainting_max_mask_radius",
                    "min_inpainting_radius",
