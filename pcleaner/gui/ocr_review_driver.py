...
import pcleaner.ai_translator as ait
import pcleaner.config as cfg
from pcleaner.gui.ai_dialog import AIDialog
...
class OcrReviewWindow(Qw.QDialog, Ui_OcrReview):
    ...
    def __init__(
        ...
        # Add Translation and Delete columns
        self.tableWidget_ocr.setColumnCount(4)
        self.tableWidget_ocr.setHorizontalHeaderItem(2, Qw.QTableWidgetItem(tr("Translation")))
        self.tableWidget_ocr.setHorizontalHeaderItem(3, Qw.QTableWidgetItem(tr("Delete")))

        # Fix visibility of flat buttons
        buttons_to_fix = [
            self.pushButton_zoom_in, self.pushButton_zoom_out, self.pushButton_zoom_reset, self.pushButton_zoom_fit,
            self.pushButton_row_up, self.pushButton_row_down, self.pushButton_new, self.pushButton_delete,
            self.pushButton_undelete, self.pushButton_reset, self.pushButton_prev, self.pushButton_next
        ]
        for btn in buttons_to_fix:
            btn.setFlat(False)
            btn.setMinimumHeight(28)
            # Add a slight border or background if the theme is problematic
            btn.setStyleSheet("QPushButton { border: 1px solid #888; border-radius: 4px; padding: 2px; } QPushButton:hover { background-color: rgba(128,128,128,0.2); }")

        # Set the table to only allow editing in the text and translation columns.
        self.tableWidget_ocr.setEditableColumns([1, 2])
        self.tableWidget_ocr.setItemDelegate(WrapTextDelegate())
        self.tableWidget_ocr.verticalHeader().setSectionResizeMode(Qw.QHeaderView.ResizeToContents)
        ...
        # Add Import Translations button as split menu for Standard or AI matching
        self.pushButton_import = Qw.QPushButton(self)
        self.pushButton_import.setText(tr("Import Translations"))
        self.pushButton_import.setFlat(True)
        
        import_menu = Qw.QMenu(self)
        action_seq = import_menu.addAction(tr("Standard (Sequential)"))
        action_seq.triggered.connect(self.import_translations)
        action_ai = import_menu.addAction(tr("Intelligent AI Match (Gemini)"))
        action_ai.triggered.connect(self.import_translations_ai)
        self.pushButton_import.setMenu(import_menu)

        self.horizontalLayout.insertWidget(self.horizontalLayout.count() - 1, self.pushButton_import)
    ...
    def init_shortcuts(self) -> None:
        ...
        set_button_shortcut(self.pushButton_import, Qg.QKeySequence(Qc.Qt.CTRL + Qc.Qt.Key_I))
    ...
    def load_ocr_results(self, ocr_results: list[st.OCRResult], preserve_scroll: bool = False) -> None:
        """
        Load the OCR results into the text fields.

        :param ocr_results: The OCR results to load for this image.
        :param preserve_scroll: If True, maintains the scrollbar position after reloading.
        """
        scroll_bar = self.tableWidget_ocr.verticalScrollBar()
        scroll_pos = scroll_bar.value() if preserve_scroll and scroll_bar else 0
        ...
        for index, ocr_result in enumerate(ocr_results):
            self.tableWidget_ocr.appendRow(
                ...
                ocr_result.text,
                ocr_result.translation,
                "",  # Placeholder for Delete button
            )

            # Add Delete button to the 4th column
            delete_btn = Qw.QPushButton("✖")
            delete_btn.setToolTip(tr("Delete this bubble"))
            delete_btn.setFixedSize(30, 30)
            delete_btn.setFlat(True)
            delete_btn.setStyleSheet("color: #d32f2f; font-weight: bold; font-size: 16px;")
            delete_btn.clicked.connect(partial(self.delete_bubble_at_row, index))
            ...
            self.tableWidget_ocr.setCellWidget(index, 3, container)

            # If the status is edited, make it italic, removed is strikeout, and
            # editedremoved is both.
            for col in range(3):  # Apply to Label, Text, and Translation
                item = self.tableWidget_ocr.item(index, col)
        ...
        if preserve_scroll and scroll_bar:
    ...
    def ocr_worker_result(self, result: tuple[str, int, int]) -> None:
        ...
        self.load_ocr_results(ocr_results, preserve_scroll=True)
    ...
    def reset_all_bubbles(self) -> None:
        ...
        self.load_ocr_results(original_results, preserve_scroll=True)
    ...
    def handle_table_edited(self, row: int, col: int) -> None:
        ...
        if col not in (1, 2):
            logger.error("Only the text and translation columns should be editable.")
            return

        ocr_results = self.current_image_ocr_results()
        ocr_results[row].text = self.tableWidget_ocr.item(row, 1).text()
        ocr_results[row].translation = self.tableWidget_ocr.item(row, 2).text()
        ...
        self.load_ocr_results(ocr_results, preserve_scroll=True)
        self.tableWidget_ocr.setCurrentCell(current_row, col)

    def import_translations(self) -> None:
        """
        Import translations from a text file and apply them to the visible bubbles.
        """
        ...
        try:
            ...
            gu.show_exception(self, tr("Error"), tr("Failed to read file:"), e)
            return

        if not lines:
            gu.show_warning(self, tr("Import Error"), tr("The selected file is empty."))
            return

        ocr_results = self.current_image_ocr_results()
        # Map to visible results only.
        visible_results = [
            r
        ...
        if not visible_results:
            gu.show_warning(self, tr("Import Error"), tr("No visible bubbles to translate."))
            return

        for i, res in enumerate(visible_results):
            if i < len(lines):
                res.translation = lines[i]
                if res.status == st.OCRStatus.Normal:
                    res.status = st.OCRStatus.Edited

        self.load_ocr_results(ocr_results, preserve_scroll=True)

    def import_translations_ai(self) -> None:
        """
        Uses Google Gemini to perform AI intelligent semantic matching across either
        the current image or the entire batch of images.
        """
        saved_key = self.config.gemini_api_key if self.config else None
        dlg = AIDialog(self, saved_key=saved_key)
        
        if dlg.exec() != Qw.QDialog.Accepted:
            return

        file_path, api_key, do_batch = dlg.get_inputs()
        ...
        try:
            ...
            gu.show_exception(self, tr("Error"), tr("Failed to read Arabic file:"), e)
            return
            
        if not arabic_text:
            gu.show_warning(self, tr("Empty File"), tr("Translation text file is empty."))
            return

        # 2. Collect English Bubbles
        # Format: { "imgIDX_bubbleIDX": "English text" }
        english_mapping = {}
        
        if do_batch:
            # Process ALL images
            for img_i, img_res_list in enumerate(self.ocr_results):
                for bub_i, res in enumerate(img_res_list):
                    if res.status not in (st.OCRStatus.Removed, st.OCRStatus.EditedRemoved) and res.text.strip():
                        key_tag = f"{img_i}_{bub_i}"
                        english_mapping[key_tag] = res.text.strip()
        else:
            # Process current image only
            cur_idx = self.image_list.currentRow()
            cur_res_list = self.ocr_results[cur_idx]
            for bub_i, res in enumerate(cur_res_list):
                if res.status not in (st.OCRStatus.Removed, st.OCRStatus.EditedRemoved) and res.text.strip():
                    key_tag = f"{cur_idx}_{bub_i}"
                    english_mapping[key_tag] = res.text.strip()

        if not english_mapping:
            gu.show_warning(self, tr("No Bubbles"), tr("Found no usable extracted English bubbles to map."))
            return

        # 3. Spawn Worker
        Qw.QApplication.setOverrideCursor(Qc.Qt.WaitCursor)
        ai_worker = wt.Worker(
            ait.perform_ai_semantic_match, english_mapping, arabic_text, api_key, no_progress_callback=True
        )
        ai_worker.signals.error.connect(self.ai_match_worker_error)
        ai_worker.signals.result.connect(self.ai_match_worker_result)
        ai_worker.signals.finished.connect(self.ocr_worker_finished) # Reuses generic cursor restore
        Qc.QThreadPool.globalInstance().start(ai_worker)

    def ai_match_worker_error(self, error: wt.WorkerError) -> None:
        logger.error(f"AI Matching worker thread error: {error}")
        gu.show_exception(self, self.tr("AI Error"), self.tr("Encountered an error with Gemini API:"), error)

    @suppress_cell_update_handling
    def ai_match_worker_result(self, mapping_results: dict) -> None:
        if not isinstance(mapping_results, dict):
             gu.show_warning(self, tr("Invalid Data"), tr("The AI didn't return a proper JSON dictionary."))
             return
             
        logger.info(f"Applying {len(mapping_results)} AI translation matches.")
        
        matches_applied = 0
        for key_tag, translated_text in mapping_results.items():
            try:
                parts = key_tag.split("_")
                img_i = int(parts[0])
                bub_i = int(parts[1])
                
                res = self.ocr_results[img_i][bub_i]
                
                if translated_text and isinstance(translated_text, str):
                     res.translation = translated_text.strip()
                     if res.status == st.OCRStatus.Normal:
                         res.status = st.OCRStatus.Edited
                     matches_applied += 1
            except (IndexError, ValueError, KeyError):
                logger.warning(f"LLM returned unrecognized mapping key: {key_tag}")
                continue

        # Reload UI for current image to reflect any immediate batch visual updates
        current_image_results = self.current_image_ocr_results()
        self.load_ocr_results(current_image_results, preserve_scroll=True)
        
        gu.show_info(self, tr("AI Match Complete"), tr("Successfully matched and applied {count} bubble translations.").format(count=matches_applied))
    ...
    def delete_bubble_at_row(self, row: int) -> None:
        """
        Delete the bubble at the specified row.
        """
        self.tableWidget_ocr.setCurrentCell(row, 0)
        self.delete_bubble()
