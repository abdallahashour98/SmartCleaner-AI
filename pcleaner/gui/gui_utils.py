...
def apply_restored_badge(pixmap: Qg.QPixmap) -> Qg.QPixmap:
    """
    Apply a subtle 'Restored' badge to the corner of the pixmap.

    :param pixmap: The source pixmap.
    :return: A new pixmap with the badge.
    """
    result = pixmap.copy()
    painter = Qg.QPainter(result)
    painter.setRenderHint(Qg.QPainter.Antialiasing)

    # Calculate badge size relative to pixmap.
    badge_width = result.width() * 0.4
    badge_height = result.height() * 0.15
    margin = result.width() * 0.05

    badge_rect = Qc.QRectF(
        result.width() - badge_width - margin,
        margin,
        badge_width,
        badge_height
    )
    ...
    # Use a nice green or blue instead of red. Green for 'Restored' (Original).
    # Actually, the user might prefer a neutral color or a brand color.
    # Let's use a subtle dark semi-transparent background with white text.
    painter.fillPath(path, Qg.QColor(0, 0, 0, 180))
    
    # Draw border.
    painter.setPen(Qg.QPen(Qg.QColor(255, 255, 255, 100), 1))
    painter.drawPath(path)

    # Draw 'ORIGINAL' text.
    font = painter.font()
    font.setBold(True)
    font.setPointSizeF(badge_height * 0.6)
    painter.setFont(font)
    painter.setPen(Qg.QColor(255, 255, 255))
    painter.drawText(badge_rect, Qc.Qt.AlignCenter, tr("ORIGINAL"))

    painter.end()
    return result


def apply_modified_badge(pixmap: Qg.QPixmap, num_cleaned: int) -> Qg.QPixmap:
    """
    Apply a subtle numeric badge to the corner of the pixmap indicating the number of cleaned regions.

    :param pixmap: The source pixmap.
    :param num_cleaned: The number of regions that were cleaned.
    :return: A new pixmap with the badge.
    """
    if num_cleaned <= 0:
        return pixmap
    ...
    # Calculate badge size relative to pixmap.
    badge_height = result.height() * 0.18
    margin = result.width() * 0.05

    badge_rect = Qc.QRectF(
        result.width() - badge_height - margin,
        margin,
        badge_height,
        badge_height
    )
    ...
    # Use a green color to indicate cleaned
    painter.fillPath(path, Qg.QColor(46, 204, 113, 200))
    ...
    font.setPointSizeF(badge_height * 0.5)
    painter.setFont(font)
    painter.setPen(Qg.QColor(255, 255, 255))
    painter.drawText(badge_rect, Qc.Qt.AlignCenter, str(num_cleaned))
