...
class OutputReviewWindow(Qw.QWidget, Ui_OutputReview):
        ...
        for image in self.images:
            ...
            try:
                self.image_list.addItem(
                    Qw.QListWidgetItem(str(original_path.stem))
                )
                self.update_list_item_icon(self.image_list.count() - 1, image)
            except OSError:
                gu.show_exception(
                    self,
    ...
    def init_cancel_button(self) -> None:
        """
        Add a cancel button to the UI.
        """
        self.pushButton_cancel = Qw.QPushButton(self)
        self.pushButton_cancel.setObjectName(u"pushButton_cancel")
        self.pushButton_cancel.setText(self.tr("Restore Original"))
        # Insert before the 'Done' button.
        self.horizontalLayout.insertWidget(self.horizontalLayout.indexOf(self.pushButton_done), self.pushButton_cancel)
        self.pushButton_cancel.clicked.connect(self.toggle_canceled)
    ...
    def init_image_viewers(self) -> None:
        ...
        # Install event filters to catch Ctrl + Right Click for Restore Original
        viewers = [
            self.image_viewer_sbs_master,
            self.image_viewer_sbs_slave,
            self.image_viewer_swipe,
            self.image_viewer_onion,
            self.image_viewer_difference,
            self.image_viewer_overlay,
        ]
    ...
    def eventFilter(self, obj: Qc.QObject, event: Qc.QEvent) -> bool:
        if event.type() == Qc.QEvent.MouseButtonPress:
            if event.button() == Qc.Qt.RightButton and event.modifiers() & Qc.Qt.ControlModifier:
                self.toggle_canceled()
                return True
    ...
    def handle_image_change(
        ...
        self.update_cancel_button_text(image)
        self.switch_to_image(image)

    def update_cancel_button_text(self, image: imf.ImageFile) -> None:
        """
        Update the text of the cancel button based on the image's status.
        """
        if image.canceled:
            self.pushButton_cancel.setText(self.tr("Cancel Restoration"))
        else:
            self.pushButton_cancel.setText(self.tr("Restore Original"))

    @Slot()
    def toggle_canceled(self) -> None:
        """
        Toggle the canceled status of the current image.
        """
        index = self.image_list.currentRow()
        if index < 0:
            return
        image = self.images[index]
        image.canceled = not image.canceled
        self.update_cancel_button_text(image)
        self.update_list_item_icon(index, image)

    def update_list_item_icon(self, index: int, image: imf.ImageFile) -> None:
        """
        Update the icon of the list item to reflect the canceled status.
        """
        item = self.image_list.item(index)
        output_path = image.outputs[self.target_output].path
        pixmap = Qg.QPixmap(str(output_path))
        
        # Use a reasonably large size for the icon so it stays sharp when scaled by the slider.
        # Qt's QIcon handles scaling it down to the list's iconSize.
        base_size = 256
        pixmap = pixmap.scaled(base_size, base_size, Qc.Qt.KeepAspectRatio, Qc.Qt.SmoothTransformation)

        if image.canceled:
            pixmap = gu.apply_restored_badge(pixmap)
        elif image.analytics_data:
            num_cleaned = 0
            for cat in [
                ost.ImageAnalyticCategory.mask_perfect,
                ost.ImageAnalyticCategory.denoised,
                ost.ImageAnalyticCategory.inpainted,
            ]:
                val_str = image.analytics_data.get_category(cat)
                if val_str:
                    try:
                        num_cleaned += int(val_str.split("/")[0])
                    except (ValueError, IndexError):
                        pass
            if num_cleaned > 0:
                pixmap = gu.apply_modified_badge(pixmap, num_cleaned)
        
        item.setIcon(Qg.QIcon(pixmap))
    ...
    def switch_to_image(self, image: imf.ImageFile) -> None:
        ...
        isolated_text_output = image.outputs.get(ost.Output.isolated_text)
        isolated_text_path = isolated_text_output.path if isolated_text_output else None
