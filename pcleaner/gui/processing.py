...
def generate_output(
    ...
    if target_output > ost.get_output_representing_step(ost.Step.text_detection):
        ...
        if step_preprocessor_images:
            ...
            for image_obj in step_preprocessor_images:
                ...
                if ost.Output.clean_json in target_outputs:
                    # If we want the text data, we need to ensure that ALL boxes are OCRed,
                    # not just the small ones that are checked for symbols.
                    ...
                    # Convert to TypeR format and save to cache.
                    if ocr_analytic is not None:
                        typer_json = ocr.format_output(
                            [ocr_analytic],
                            "json",
                            (tr("filename"), tr("startx"), tr("starty"), tr("endx"), tr("endy"), tr("text")),
                        )
                        cache_export_json = path_gen._attach("#export.json")
                        cache_export_json.write_text(typer_json, encoding="utf-8")
                else:
                    ocr_analytic = pp.prep_json_file(
                        path_gen.raw_json,
                        preprocessor_conf=profile.preprocessor,
        ...
        if step_masker_images:
            ...
            if pool_size > 1:
                with Pool(processes=pool_size) as pool:
                    iterator = pool.imap(ma.mask_page, data)
                    for _ in range(len(data)):
                        check_abortion()
                        try:
                            analytic = next(iterator)
                            masker_analytics_raw.extend(analytic)
                        except StopIteration:
                            break
                        except Exception as e:
                            logger.error(f"Masker failed for an image, skipping: {e}")
    ...
    if (
        ...
        if step_denoiser_images:
            ...
            if pool_size > 1:
                with Pool() as pool:
                    iterator = pool.imap(dn.denoise_page, data)
                    for _ in range(len(data)):
                        check_abortion()
                        try:
                            analytic = next(iterator)
                            ...
                            logger.error(f"Denoiser failed for an image, skipping: {e}")
        ...
        if step_inpainting_images:
            ...
            for data_obj in data:
                ...
                try:
                    ...
                    logger.error(f"Inpainter failed for an image, skipping: {e}")
    ...
    if target_output == ost.Output.write_output:
        ...
        handle_merging_splits(image_objects, split_files, profile, cache_dir, ocr_analytics=ocr_analytics, target_outputs=target_outputs)
        batch_metadata.set_input_paths_from_files(image_objects)

        # If we want a single merged JSON, generate it now.
        if not individual_json and ost.Output.clean_json in target_outputs and ocr_analytics:
            logger.info("Generating merged JSON output...")
            merged_json = ocr.format_output(
                ocr_analytics,
                "json",
                (tr("filename"), tr("startx"), tr("starty"), tr("endx"), tr("endy"), tr("text")),
            )
            ...
            if output_dir:
                merged_json_path = output_dir / "all_text.json"
                merged_json_path.write_text(merged_json, encoding="utf-8")
                batch_metadata.output_files.append(merged_json_path)
        ...
        export_outputs = list(target_outputs)
        if not individual_json and ost.Output.clean_json in export_outputs:
            export_outputs.remove(ost.Output.clean_json)
        ...
        handle_merging_splits(image_objects_export, split_files, profile, cache_dir, ocr_analytics=ocr_analytics, target_outputs=target_outputs)
...
def perform_ocr(
    ...
    handle_merging_splits(image_objects, split_files, profile, cache_dir, for_ocr=True, ocr_analytics=ocr_analytics, target_outputs=target_outputs)

    check_abortion()

    # Output the OCRed text from the analytics.
    if individual_json and output_file is not None and not csv_output:
        # Save individual JSON files in the same directory as the output_file would be,
        # but one for each image.
        ...
        for analytic in ocr_analytics:
            ...
            try:
                ...
                individual_output_file.write_text(text_out, encoding="utf-8")
            except OSError:
                logger.error(f"Failed to write individual JSON to {individual_output_file}")
        
        text_out = tr("Saved individual JSON files to {output_dir}").format(output_dir=output_dir)
    else:
        text_out = ocr.format_output(
            ocr_analytics,
            "csv" if csv_output else "json" if str(output_file).endswith(".json") else "plain",
            (tr("filename"), tr("startx"), tr("starty"), tr("endx"), tr("endy"), tr("text")),
        )
...
def handle_merging_splits(
    ...
    for_ocr: bool = False,
    ocr_analytics: list[st.OCRAnalytic] = None,
    target_outputs: Sequence[ost.Output] = (),
):
    """
    ...
    if profile.general.merge_after_split:
        ...
        for split_file in split_files:
            ...
            for split_object in split_objects:
                if split_object in image_objects:
                    image_objects.remove(split_object)

            # If we are extracting text, we need to save the merged JSON to the new object's cache.
            if ost.Output.clean_json in target_outputs and ocr_analytics:
                # Find the merged analytic for this image.
                merged_analytic = next((a for a in ocr_analytics if a.path == split_file), None)
                if merged_analytic:
                    typer_json = ocr.format_output(
                        [merged_analytic],
                        "json",
                    ...
                    path_gen = ost.OutputPathGenerator(merged_object.path, cache_dir, merged_object.uuid)
                    cache_export_json = path_gen._attach("#export.json")
                    cache_export_json.write_text(typer_json, encoding="utf-8")
...
def handle_merging_ocr_splits(
    ...
    if profile.general.merge_after_split:
        ...
        for split_file in split_files:
            # Check if this file has already been merged in ocr_analytics.
            if any(a.path == split_file for a in ocr_analytics):
                continue
