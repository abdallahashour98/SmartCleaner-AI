...
class MainWindow(Qw.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, config: cfg.Config, files_to_open: list[str], debug: bool) -> None:
        ...
        # Register state saver AFTER initialize_ui(), since some widgets
        # (radioButton_ocr_json, checkBox_ocr_individual) are created dynamically there.
    ...
    def initialize_ui(self) -> None:
        ...
        # Add JSON radio button
        self.radioButton_ocr_json = Qw.QRadioButton(self.page_ocr)
        self.radioButton_ocr_json.setObjectName(u"radioButton_ocr_json")
        self.radioButton_ocr_json.setText("JSON (TypeR)")
        self.verticalLayout_15.addWidget(self.radioButton_ocr_json)

        self.radioButton_ocr_text.clicked.connect(partial(self.handle_ocr_mode_change, mode="plain"))
        self.radioButton_ocr_csv.clicked.connect(partial(self.handle_ocr_mode_change, mode="csv"))
        self.radioButton_ocr_json.clicked.connect(partial(self.handle_ocr_mode_change, mode="json"))

        # Add Individual JSON checkbox to OCR tab
        self.checkBox_ocr_individual = Qw.QCheckBox(self.page_ocr)
        self.checkBox_ocr_individual.setObjectName(u"checkBox_ocr_individual")
        self.checkBox_ocr_individual.setText(self.tr("Individual JSON per image"))
        self.checkBox_ocr_individual.setChecked(True)
        self.verticalLayout_15.addWidget(self.checkBox_ocr_individual)

        # Add Extract Text Data checkbox to Cleaning tab
        self.checkBox_save_ocr = Qw.QCheckBox(self.page_cleaning)
        self.checkBox_save_ocr.setObjectName(u"checkBox_save_ocr")
        self.checkBox_save_ocr.setText(self.tr("Extract Text (TypeR JSON)"))
        self.verticalLayout_11.addWidget(self.checkBox_save_ocr)

        self.checkBox_cleaning_individual = Qw.QCheckBox(self.page_cleaning)
        self.checkBox_cleaning_individual.setObjectName(u"checkBox_cleaning_individual")
        self.checkBox_cleaning_individual.setText(self.tr("Individual JSON per image"))
        self.verticalLayout_11.addWidget(self.checkBox_cleaning_individual)
        self.checkBox_cleaning_individual.setVisible(False)

        # Connect individual checkbox visibility
        self.checkBox_save_ocr.toggled.connect(self.checkBox_cleaning_individual.setVisible)
        
        # Automatically calculate output path based on input files and options (Now hooked up safely after construction)
        self.file_table.table_not_empty.connect(self.auto_fill_output_path)
        self.checkBox_save_clean.toggled.connect(self.auto_fill_output_path)
        self.checkBox_save_ocr.toggled.connect(self.auto_fill_output_path)
        ...
        self.menu_session = self.menuBar().addMenu(self.tr("Session"))
        self.action_save_session = Qg.QAction(self.tr("Save Session"), self)
        self.action_load_session = Qg.QAction(self.tr("Load Session"), self)
        self.menu_session.addAction(self.action_save_session)
        self.menu_session.addAction(self.action_load_session)
    ...
    def post_init(self) -> None:
        ...
        # Update OCR mode UI based on restored state
        if self.radioButton_ocr_json.isChecked():
            self.handle_ocr_mode_change("json")
        elif self.radioButton_ocr_csv.isChecked():
            self.handle_ocr_mode_change("csv")
        else:
            self.handle_ocr_mode_change("plain")
    ...
    def auto_fill_output_path(self) -> None:
        """
        Automatically set output directory and file path based on image inputs path.
        """
        ...
        try:
            ...
            if not image_files:
                return
            common_path = hp.common_path_parent([f.export_path for f in image_files])
        except Exception:
            return

        # 1. Update cleaning output directory to match input exactly
        self.lineEdit_out_directory.setText(str(common_path.resolve()))
        
        # 2. Update OCR output file path to be inside the input directory
        current_out_file = self.lineEdit_out_file.text()
        if not current_out_file:
            current_out_file = self.lineEdit_out_file.placeholderText()
            if not current_out_file:
                current_out_file = "output.txt"
            
        out_file_path = Path(current_out_file)
        new_out_file = common_path.resolve() / out_file_path.name
        self.lineEdit_out_file.setText(str(new_out_file))
    ...
    def handle_ocr_mode_change(self, mode: str) -> None:
        """
        When the user changes the OCR output mode, update the file extension.
        """
        current_placeholder_path = Path(self.lineEdit_out_file.placeholderText())
        if mode == "json":
            current_placeholder_path = current_placeholder_path.with_suffix(".json")
        elif mode == "csv":
        ...
        self.checkBox_ocr_individual.setVisible(mode == "json")
        ...
        if current_text:
            try:
                ...
                if mode == "json":
                    current_path = current_path.with_suffix(".json")
                elif mode == "csv":
    ...
    def closeEvent(self, death: Qg.QCloseEvent) -> None:
        ...
        """
        if not self.file_table.has_no_files():
            response = gu.show_question(
                self,
                self.tr("Save Session?"),
                self.tr("You have files in your workspace. Do you want to save the session so you can resume later?"),
                buttons=Qw.QMessageBox.Yes | Qw.QMessageBox.No | Qw.QMessageBox.Cancel
            )
            if response == Qw.QMessageBox.Cancel:
                death.ignore()
                return
            elif response == Qw.QMessageBox.Yes:
                self.save_session()
                self._skip_cache_clean = True
            else:
                self._skip_cache_clean = False
        else:
            self._skip_cache_clean = False
        ...
        self.terminate.emit()
        if not self.debug and not self._skip_cache_clean:
    ...
    def init_state_saver(self):
        """
        Register widgets with the state saver.
        """
        self.state_saver.register(self, "geometry", "windowState")
        ...
        self.state_saver.register(
            ...
            self.radioButton_ocr_json,
            self.checkBox_review_ocr,
            self.checkBox_ocr_individual,
        )
    ...
    def save_session(self) -> None:
        ...
        """
        import pcleaner.gui.session_manager as sm
        if self.file_table.has_no_files():
            gu.show_info(self, self.tr("No files"), self.tr("There are no files to save."))
            return
            
        success = sm.save_session(self.file_table.get_image_files(), self.config.get_cleaner_cache_dir())
        if success:
            gu.show_info(self, self.tr("Session Saved"), self.tr("Session saved successfully."))
        else:
            gu.show_warning(self, self.tr("Save Failed"), self.tr("Failed to save the session."))
    ...
    def load_session(self) -> None:
        ...
        import pcleaner.gui.session_manager as sm
        import pcleaner.gui.image_file as imf
        session_data = sm.load_session(self.config.get_cleaner_cache_dir())
        if not session_data:
            gu.show_info(self, self.tr("No Session"), self.tr("No previous session found."))
            return
        ...
        for data in session_data:
            path = Path(data["path"])
            if not path.exists():
                logger.warning(f"File {path} no longer exists, skipping.")
                # Allow loading anyway since it might be a cached split file
            
            split_from = Path(data["split_from"]) if data.get("split_from") else None
            export_path = Path(data["export_path"]) if data.get("export_path") else path
            
            img = imf.ImageFile(path=path, split_from=split_from, export_path=export_path)
            self.file_table.files[path] = img
            if split_from:
                if split_from not in self.file_table.split_files:
                    self.file_table.split_files[split_from] = []
                self.file_table.split_files[split_from].append(img)
                
        sm.apply_session_data_to_images(
            session_data, 
            self.file_table.files,
            self.config.current_profile,
            self.config.get_cleaner_cache_dir()
        )
        self.file_table.repopulate_table()
        self.file_table.update_all_rows()
        gu.show_info(self, self.tr("Session Loaded"), self.tr("Session loaded successfully."))
    ...
    def clean_cache(self, force: bool = False) -> None:
        ...
        if not force and sm.has_session(cache_dir):
            logger.info("A saved session exists. Skipping cache clean on startup to preserve data.")
            return
    ...
    def start_cleaning(self) -> None:
        ...
        request_ocr = self.checkBox_save_ocr.isChecked()
        individual_json = self.checkBox_cleaning_individual.isChecked()
        ...
        if request_ocr:
            requested_outputs.append(ost.Output.clean_json)
    ...
    def post_review_export(self) -> None:
        ...
        worker = wt.Worker(
            ...
            ocr_analytics=self.cleaning_review_options.ocr_analytics,
            individual_json=self.cleaning_review_options.individual_json,
            abort_signal=self.get_abort_signal(),
        )
        worker.signals.progress.connect(self.show_current_progress)
        worker.signals.result.connect(self.output_worker_result)
    ...
    def post_review_ocr_export(self, ocr_analytics: list[st.OCRAnalytic]) -> None:
        ...
        output_file_or_dir = self.ocr_review_options.output_path
        individual_json = getattr(self.ocr_review_options, "individual_json", False)

        output_format = "csv" if self.ocr_review_options.csv_output else "plain"
        is_cleaning_run = False
        output_file = None
        output_dir = None

        if output_file_or_dir is not None:
            if not output_file_or_dir.suffix:
                # Coming from start_cleaning, where output_path is a directory.
                is_cleaning_run = True
                output_format = "json"  # Cleaning extracts text as JSON
        ...
        if individual_json and output_dir is not None and output_format == "json":
            for analytic in ocr_analytics:
                formatted_text = ocr.format_output(
                    [analytic],
                    "json",
                    (self.tr("filename"), self.tr("startx"), self.tr("starty"), self.tr("endx"), self.tr("endy"), self.tr("text")),
                )
                individual_output_file = output_dir / analytic.path.with_suffix(".json").name
                try:
                    individual_output_file.parent.mkdir(parents=True, exist_ok=True)
                    individual_output_file.write_text(formatted_text, encoding="utf-8")
                except OSError:
                    logger.error(f"Failed to write individual JSON to {individual_output_file}")
            text_out = self.tr("Saved individual JSON files to {output_dir}").format(output_dir=output_dir)
        else:
            ...
            # For merged output in cleaning run, save as all_text.json
            if is_cleaning_run and output_dir is not None:
                output_file = output_dir / "all_text.json"

            if output_file is not None:
                try:
                    output_file.parent.mkdir(parents=True, exist_ok=True)
                    output_file.write_text(text_out, encoding="utf-8")
                    text_out += "\n\n" + self.tr("Saved detected text to {output_file}").format(
                        output_file=output_file
                    )
                except OSError:
                    text_out += "\n\n" + self.tr("Failed to write detected text to {output_file}").format(
                        output_file=output_file
                    )
                    gu.show_exception(
                        None, self.tr("Save Failed"), self.tr("Failed to write detected text to file.")
                    )
    ...
    def generate_output(
        ...
        individual_json: bool = True,
        progress_callback: ost.ProgressSignal = None,
        abort_flag: wt.SharableFlag = None,
    ) -> None:
        """
        ...
        :param ocr_analytics: [Optional] The OCR analytics to use.
    ...
    def start_ocr(self) -> None:
        ...
        """
        # Check if we're outputting as plain text, csv or json file.
        csv_output = self.radioButton_ocr_csv.isChecked()
        json_output = self.radioButton_ocr_json.isChecked()
        review_ocr = self.checkBox_review_ocr.isChecked()
        individual_json = self.checkBox_ocr_individual.isChecked()

        try:
            output_path = Path(self.lineEdit_out_file.text())
            # Respect .json if explicitly provided or radio button is checked.
            if json_output:
        ...
        if review_ocr:
            ...
            self.ocr_review_options = gst.OcrReviewOptions(
                image_files, [], output_path, csv_output, editing_old_data=False, individual_json=individual_json
            )
    ...
    def output_review_closed(self) -> None:
        ...
        if self.cleaning_review_options.export_afterwards and (
            ...
            self.cleaning_review_options = None
            self.enable_running_cleaner()
            # If cleaning review is bypassed or export skipped, still launch OCR review if requested.
            if self.ocr_review_options:
                self.launch_ocr_review()
    ...
    def launch_ocr_review(self) -> None:
        """
        Launch the OCR review dialog modal.
        Extracted as a reusable method to support chaining from cleaning review workflows.
        """
        if not self.ocr_review_options:
            return

        logger.info("Launching OCR review window...")
        ...
        # Finally run post actions if they are pending now that all cascades have finished.
        if self.current_post_action is not None:
            self.wait_to_run_post_action()
            self.cancel_post_action()
        else:
    ...
    def output_worker_aborted(self) -> None:
        ...
        if hasattr(self, "label_stats"):
            self.label_stats.setText(self.tr("Processing aborted."))
    ...
    def output_worker_finished(self) -> None:
        ...
        if hasattr(self, "label_stats"):
            self.label_stats.setText(self.tr("Processing finished."))
    ...
    def output_worker_error(self, error: wt.WorkerError) -> None:
        ...
        self.ocr_review_options = None
        if hasattr(self, "label_stats"):
            self.label_stats.setText(self.tr("Processing error."))
    ...
    def show_current_progress(self, progress_data: ost.ProgressData) -> None:
        ...
        if progress_data.progress_type == ost.ProgressType.start:
            ...
            self.label_stats.setText(self.tr("Processing started..."))
            return
            ...
            if self.cleaning_review_options:
                self.cleaning_review_options.ocr_analytics = ocr_analytics
            if self.ocr_review_options:
                self.ocr_review_options.ocr_results = ocr_analytics
        ...
        if hasattr(self, "processing_start_time") and hasattr(self, "step_start_time"):
            total_elapsed = time.time() - self.processing_start_time
            ...
            elapsed_str = f"{self.tr('Elapsed')}: {format_time(total_elapsed)}"
            
            if self.progress_current > 0 and progress_data.total_images > 0:
                elapsed_in_step = time.time() - self.step_start_time
                time_per_image = elapsed_in_step / self.progress_current
                remaining_in_step = time_per_image * (progress_data.total_images - self.progress_current)
                eta_str = f" | {self.tr('Step ETA')}: {format_time(remaining_in_step)}"
            else:
                eta_str = f" | {self.tr('Step ETA')}: --:--"
                
            self.label_stats.setText(elapsed_str + eta_str)
