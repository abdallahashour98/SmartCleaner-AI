...
def show_ocr_analytics(
    ...
    for analytic in analytics:
        for text, box, translation in analytic.removed_box_data:
...
def show_inpainting_analytics(
    ...
    if analytics:
        # Show the threshold and radii from the first page (or average them if they vary).
        # Since they are likely very similar across pages in a batch, showing the first is a good start.
        first = analytics[0]
        buffer.write(
            tr("Auto Mode (v3 - Aggressive)")
            + f": {first.threshold:.2f} | "
            + tr("Min Radius")
            + f": {first.min_inp_radius}px\n"
        )
        buffer.write(
            tr("Isolation Radius")
            + f": {first.isolation_radius}px | "
            + tr("Fade Radius")
            + f": {first.fade_radius}px\n"
        )
