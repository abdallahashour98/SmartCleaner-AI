...
class OCRAnalytic:
    ...
    - the cached file name and the text, the box, and the translation that was removed.
    """
    ...
    removed_box_data: Sequence[tuple[str, Box, str]]
    all_box_data: Sequence[tuple[str, Box, str]] = ()
...
class OCRResult:
    ...
    bubbles: list[tuple[Path, str, Box, str, OCRStatus, str]]
    """
...
def convert_ocr_analytics_to_results(ocr_analytics: list[OCRAnalytic]) -> list[list[OCRResult]]:
    ...
    for analytic in ocr_analytics:
        bubbles = [
            OCRResult(analytic.path, text, box, str(index), OCRStatus.Normal, translation)
            for index, (text, box, translation) in enumerate(analytic.removed_box_data, start=1)
        ]
...
def convert_ocr_results_to_analytics(ocr_results: list[list[OCRResult]]) -> list[OCRAnalytic]:
    ...
    for results in ocr_results:
        keeping_box_data = [
            (result.text, result.box, result.translation)
...
def merge_ocr_analytics(
    ...
    offset = 0
    for segment_path in segment_paths:
        analytic = split_file_to_analytic[segment_path]
        for text, box, translation in analytic.removed_box_data:
            new_box = box.translate(0, offset)
            new_box_tuples.append((text, new_box, translation))
        for text, box, translation in analytic.all_box_data:
            new_box = box.translate(0, offset)
            new_all_box_tuples.append((text, new_box, translation))
