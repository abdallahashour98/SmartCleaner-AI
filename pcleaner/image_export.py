...
def copy_to_output(
    ...
    # Preload the original image to get its size and metadata.
    original_image = Image.open(original_image_path)
    original_size = original_image.size

    if canceled:
        # If canceled, we want the "cleaned" output to be the original image.
        # We also clear other outputs because they don't make sense for a canceled image.
        outputs = [ost.Output.masked_output]
    ...
    if ost.Output.masked_output in outputs:
        source = original_image_path if canceled else cache_path_gen.clean
        export_single_image(
            source,
            cleaned_out_path,
            original_image_path,
    ...
    if ost.Output.clean_json in outputs:
        # Export the clean_json file to the output directory.
        # We use the TypeR format we saved in the cache as #export.json.
        cache_export_json = cache_path_gen._attach("#export.json")
        if cache_export_json.exists():
            json_out_path = cleaned_out_path.with_suffix(".json")
            shutil.copy2(cache_export_json, json_out_path)
...
def merge_cached_images(
    ...
    for target in generator_targets:
        # Resolve the path for this target on the master generator.
        try:
            path_to = getattr(path_gen_master, target)
        except AttributeError:
            # If the target is not a property, try to use for_output if it matches an enum name.
            try:
                out_enum = ost.Output[target]
                path_to = path_gen_master.for_output(out_enum)
            except (KeyError, AttributeError):
                continue

        # Check if ANY split has this output.
        if not any(getattr(gen, target).exists() for gen in file_generators if hasattr(gen, target)):
            continue

        paths_from = []
        temp_files = []
        for i, image in enumerate(image_files):
            gen = file_generators[i]
            p = getattr(gen, target) if hasattr(gen, target) else None
            
            # Determine which path to use for this segment.
            seg_path = None
            if image.canceled:
                if target in ("clean", "clean_denoised", "clean_inpaint"):
                    seg_path = image.original_path
                # For masks/overlays, we'll fall through to the blank image creation logic.
            elif p and p.exists():
                seg_path = p

            if seg_path:
                paths_from.append(seg_path)
            elif target in ("inpainting", "noise_mask", "combined_mask", "text", "boxes", "base_png"):
                # Create a blank image of the correct size.
                # We can get the size from the original split image.
                try:
                    split_img = Image.open(image.original_path)
                    blank = Image.new("RGBA" if target == "text" else "L", split_img.size, 0)
                    temp_path = cache_dir / f"temp_blank_{uuid}_{i}_{target}.png"
                    blank.save(temp_path)
                    paths_from.append(temp_path)
                    temp_files.append(temp_path)
                except Exception as e:
                    logger.error(f"Failed to create blank image for {target}: {e}")
                    paths_from = []
                    break
            else:
                # For unknown targets or if we couldn't resolve a path, abort this target.
                paths_from = []
                break
