...
def run_cleaner(
    ...
    if not skip_pre_processing:  # Pre-Processing ==================================================
        ...
        try:
            import torch
            torch.set_num_threads(max(1, (os.cpu_count() or 1) // max_workers))
        except ImportError:
            pass
            
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            for ocr_analytics_of_a_page in tqdm(executor.map(process_json, json_files), total=len(json_files)):
    ...
    if not skip_inpainting:  # Inpainting ==========================================================
        ...
        # Check the size of the required pool
        max_workers = min(4, os.cpu_count() or 1)
        ...
        def process_inpaint(inpaint_data):
            return ip.inpaint_page(inpaint_data, inpainter_model)
            
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            for analytic in tqdm(executor.map(process_inpaint, data), total=len(data)):
